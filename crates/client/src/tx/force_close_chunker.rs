...
pub async fn force_close_chunker(
    client: &RpcClient,
    payer: &Keypair,
    chunker: Pubkey,
    fee_strategy: FeeStrategy,
) -> Result<Message, Error> {
    let accounts = chunker::accounts::ForceCloseChunker {
        chunker,
        signer: payer.pubkey(),
        system_program: system_program::ID,
    };

    let data = chunker::instruction::ForceCloseChunker {};
...
mod tests {
    ...
    async fn verify() {
        ...
        let msg = force_close_chunker(&client, &payer, chunker.pubkey(), FeeStrategy::default())
            .await
            .unwrap();
